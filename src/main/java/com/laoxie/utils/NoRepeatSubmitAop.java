...
public class NoRepeatSubmitAop {
     ...
     * 重复提交判断时间为2s
     */
    private final Cache<String, Integer> cache = CacheBuilder.newBuilder().expireAfterWrite(5L, TimeUnit.SECONDS).build();

    //@Around("execution(* xyz.zhouzhaodong..*Controller.*(..)) && @annotation(nrs)")
    @Around("execution(* com.laoxie.controller.*.*(..)) && @annotation(nrs)")
